 ...
 * @param {number} vectorClock The value of the site's vector clock
 ...
 * @return {atomIdent}
 */
export function genAtomIdent(siteID, vectorClock, prevAtomIdent, nextAtomIdent) {
  return [genPosition(siteID, prevAtomIdent[0], nextAtomIdent[0]), vectorClock];
}

/**
 * Insert an atom into a sequence, returning it.
 *
 * @function
 ...
 * @return {sequence}
 */
export function insertAtom(sequence, atom) {
  const prev = sequence[0];
  ...
  if (comparisons[0] === 1 && comparisons[1] === -1) {
    return [prev].concat([atom]).concat([next]).concat(sequence.slice(2))
  } else if (comparisons[0] === 1 && comparisons[1] === 1) {
    return [prev].concat(insertAtom(sequence.slice(1), atom));
  } else if (comparisons[0] === -1 && comparisons[1] === 1 ||
